...
const MissionDetails = () => {
  ...
  const handleUserChange = async (userId) => {
    try {
      // Add user to the mission in the database
      const response = await axios.put(`http://localhost:3000/api/missions/${missionId}/assign`, { userId });
      setMission(response.data); // Update mission data with the new user list
      console.log(`User with ID ${userId} assigned to mission ${missionId}`);
    } catch (error) {
