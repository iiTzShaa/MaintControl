...
const MissionDetails = () => {
  ...
  const handleUserChange = async (userId) => {
    try {
      ...
      const newUser = response.data.user; // Assuming response includes the assigned user's data
      // Update mission state directly with the new user
      setMission((prevMission) => ({
        ...prevMission,
        users: [...prevMission.users, newUser],
      }));
    } catch (error) {
      console.error('Failed to assign user to mission:', error);
    }
  ...
  const releaseUserFromMission = async (userId) => {
    try {
      await axios.post(`http://localhost:3000/users/${userId}/release-mission/${missionId}`);
      // Update mission state directly to remove the user
      setMission((prevMission) => ({
        ...prevMission,
